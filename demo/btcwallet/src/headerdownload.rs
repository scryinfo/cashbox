...
use crate::timeout::{SharedTimeout, ExpectedReply};
use crate::downstream::SharedDownstream;
use std::sync::mpsc;
use std::thread;
use crate::p2p::{PeerMessageSender, P2PControlSender, PeerMessageReceiver, PeerMessage, PeerId, SERVICE_BLOCKS, P2PControl};
...
impl HeaderDownload {
    pub fn new(chaindb: SharedChainDB, p2p: P2PControlSender<NetworkMessage>, timeout: SharedTimeout<NetworkMessage, ExpectedReply>, downstream: SharedDownstream) -> PeerMessageSender<NetworkMessage> {
        ...
        let mut headerdownload = HeaderDownload { chaindb, p2p, timeout, downstream };
    ...
    fn headers(&mut self, headers: &Vec<BlockHeader>, peer: PeerId) -> Result<(), SPVError> {
