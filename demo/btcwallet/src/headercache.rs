...
impl CachedHeader {
    ...
    /// correctly.
    pub fn spv_validate(&self, required_target: &Uint256) -> Result<(), SPVError> {
        ...
        if hash <= target { Ok(()) } else { Err(SPVError::SpvBadProofOfWork) }
    }

    /// Returns the total work of the block
    pub fn work(&self) -> Uint256 {
        // 2**256 / (target + 1) == ~target / (target+1) + 1    (stolen from bitcoind)
...
impl HeaderCache {
    ...
    pub fn add_header(&mut self, header: &BlockHeader) -> Result<Option<(CachedHeader, Option<Vec<Sha256dHash>>, Option<Vec<Sha256dHash>>)>, SPVError> {
    ...
    fn add_header_to_tree(&mut self, prev: &CachedHeader, next: &BlockHeader) -> Result<(CachedHeader, Option<Vec<Sha256dHash>>, Option<Vec<Sha256dHash>>), SPVError> {
    ...
    /// about tip see detail at  [https://bitcoincore.org/en/doc/0.18.0/rpc/blockchain/getchaintips/]
    ...
    /// consensus code.
    ///
    /// details about nBits [https://zhuanlan.zhihu.com/p/51140710]
    /// precision  --》 jingque
