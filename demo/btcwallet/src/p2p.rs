...
use bitcoin::consensus::{serialize, encode, Decodable};
use futures::{Poll as Async, Future, future, FutureExt, task::Waker, TryFutureExt, SinkExt};
...
struct Peer<Message> {
    ...
    pub pid: PeerId,
    // the event poller, shared with P2P, needed here to register for events mio poll
    poll: Arc<Poll>,
...
impl<Message: Version + Send + Sync + Clone,
    ...
    pub fn new(config: Config, dispatcher: PeerMessageSender<Message>, back_pressure: usize) -> (Arc<P2P<Message, Envelope, Config>>, P2PControlSender<Message>) {
        ...
        thread::Builder::new().name("p2p_control".to_string()).spawn(move || p2p2.control_loop(control_receiver)).unwrap();
    ...
    pub fn add_peer(&self, network: &'static str, source: PeerSource) -> impl Future<Output=Result<SocketAddr, SPVError>> + Send {
    ...
    fn connecting(&self, pid: PeerId, source: PeerSource) -> impl Future<Output=Result<SocketAddr, SPVError>> + Send {
        ...
        future::poll_fn(move |_| {
            ...
            future::poll_fn(move |ctx|
                if let Some(peer) = peers2.read().unwrap().get(&pid) {
                    ...
                    Async::Ready(Err(SPVError::Handshake))
                }
    ...
    fn connect(version: Message, peers: Arc<RwLock<PeerMap<Message>>>, poll: Arc<Poll>, pid: PeerId, source: PeerSource) -> Result<SocketAddr, SPVError> {
        let outgoing;
        let addr;
        let stream;
        match source {
...
pub struct VersionCarrier {
    ...
    pub version: u32,
    /// A bit-mask describing the services supported by this node
    pub services: u64,
...
impl<Message> Peer<Message> {
    /// create a new peer
    pub fn new (pid: PeerId, stream: TcpStream, poll: Arc<Poll>, outgoing: bool) -> Result<Peer<Message>, SPVError> {
    ...
    pub fn send (&self, msg: Message) -> Result<(), SPVError> {
