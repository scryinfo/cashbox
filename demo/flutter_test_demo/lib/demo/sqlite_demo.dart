...
class SQLiteDemo {
  ...
  void initDB() async {
    // open the database
    var dbFilePath =
        "/data/data/wallet.cashbox.scry.info/files/cashbox_wallet_detail.db"; //todo
    //                /data/data/wallet.cashbox.scry.info/files/cashbox_wallet_detail.db
    database = await openDatabase(dbFilePath, version: 1,
  ...
  void testGetDbData() async {
    ...
    print(chainList);
    List<Map> defaultDigitList =
        await database.rawQuery('SELECT * FROM DefaultDigitBase');
    print(defaultDigitList);
    List<Map> metaDataList =
        await database.rawQuery('SELECT * FROM android_metadata');
    print(metaDataList);
  }
