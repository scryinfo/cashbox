...
use model::{wallet_store::TbWallet, Address, Mnemonic, TbAddress, Wallet};
...
impl WalletManager {
    ...
    pub fn reset_mnemonic_pwd(
        ...
        if let Some(mn) = provider.query_by_wallet_id(wallet_id) {
            self.mnemonic_psd_update(&mn, old_pwd, new_pwd)
                .map(|_| StatusCode::OK)
        } else {
    ...
    pub fn is_contain_wallet(&self) -> WalletResult<bool> {
        ...
        match instance.get_wallets() {
            Ok(wallets) => Ok(!wallets.is_empty()),
            Err(e) => Err(e.into()),
        }
    }
    ...
    pub fn del_wallet(&self, walletid: &str, psd: &[u8]) -> WalletResult<()> {
        ...
        //Query the mnemonic word corresponding to id
        provider
            .query_by_wallet_id(walletid)
            .ok_or_else(|| WalletError::NotExist)
            .and_then(|mn| {
                substratetx::Sr25519::get_mnemonic_context(mn.mnemonic.as_str(), psd)
                    .map_err(|e| e.into())
            })
            .and_then(|_data| provider.tx_begin())
            .and_then(|()| provider.del_mnemonic(walletid))
    ...
    pub fn rename_wallet(&self, walletid: &str, wallet_name: &str) -> WalletResult<bool> {
        ...
        if instance.query_by_wallet_id(walletid).is_some() {
            instance
                .rename_mnemonic(walletid, wallet_name)
                .map(|_| true)
        } else {
            Err(WalletError::NotExist)
        }
    ...
    pub fn create_wallet(
        ...
        if wallet_type == 1
            && dbhelper
                .query_by_wallet_digest(hex_mn_digest.as_str(), wallet_type)
                .is_some()
        {
        ...
        dbhelper
            .save_wallet_address(wallet_save, address)
    ...
    pub fn export_mnemonic(&self, wallet_id: &str, password: &[u8]) -> WalletResult<Mnemonic> {
        ...
        match provider.query_by_wallet_id(wallet_id) {
            Some(mn) => substratetx::Sr25519::get_mnemonic_context(mn.mnemonic.as_str(), password)
    ...
    fn get_wallet_info(&self) -> HashMap<String, Wallet> {
        ...
        if let Ok(mns) = instance.get_wallets() {
            for item in mns {
                let wallet = Wallet {
                    status: StatusCode::OK,
