...
pub fn show_digit(walletid: &str, chainid: i64, digitid: &str) -> WalletResult<bool> {
    let instance = wallet_db::DataServiceProvider::instance()?;
    instance.show_digit(walletid, chainid, digitid)
}

pub fn hide_digit(walletid: &str, chainid: i64, digitid: &str) -> WalletResult<bool> {
    let instance = wallet_db::DataServiceProvider::instance()?;
...
pub fn add_wallet_digit(wallet_id:&str,chain_id:i64,digit_id:&str)-> WalletResult<()>{
    let instance = wallet_db::DataServiceProvider::instance()?;
    instance.tx_begin()?;
    match instance.add_digit_from_base(wallet_id, chain_id, digit_id){
...
pub fn update_default_digit(digits:Vec<model::DefaultDigit>) ->WalletResult<()>{
