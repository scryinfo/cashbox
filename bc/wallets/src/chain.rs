...
use mav::ma::{
    Dao, EeeTokenType, MAccountInfoSyncProg, MAddress, MBtcChainToken, MBtcChainTokenAuth,
    MBtcChainTokenDefault, MBtcChainTokenShared, MEeeChainToken, MEeeChainTokenAuth,
    MEeeChainTokenDefault, MEeeChainTokenShared, MEeeChainTx, MEeeTokenxTx, MEthChainToken,
    MEthChainTokenAuth, MEthChainTokenDefault, MEthChainTokenNonAuth, MEthChainTokenShared,
    MTokenAddress, MTokenShared, MWallet,
};

use mav::{CFalse, CTrue, NetType, WalletType};
use wallets_types::{
    AccountInfo, AccountInfoSyncProg, BtcBalance, BtcChainTokenAuth, BtcChainTokenDefault,
    BtcChainTrait, BtcNowLoadBlock, BtcTxParam, Chain2WalletType, ChainTrait, ContextTrait,
    DecodeAccountInfoParameters, EeeChainTokenAuth, EeeChainTokenDefault, EeeChainTrait,
    EeeChainTx, EeeTransferPayload, EthChainTokenAuth, EthChainTokenDefault, EthChainTokenNonAuth,
    EthChainTrait, EthRawTxPayload, EthTransferPayload, ExtrinsicContext, RawTxParam,
    StorageKeyParameters, SubChainBasicInfo, WalletError, WalletTrait,
};
...
impl ChainTrait for EthChain {
    fn generate_address(
        ...
        net_type: &NetType,
    ) -> Result<MAddress, WalletError> {
        let mut m_address = MAddress::default();
        m_address.chain_type =
            wallets_types::EthChain::chain_type(wallet_type, net_type).to_string();
    ...
    async fn generate_default_token(
        &self,
        ...
        for default_token in default_tokens {
            {
                ...
                token.wallet_id = wallet.id.clone();
                token.chain_type =
                    wallets_types::EeeChain::chain_type(&wallet_type, &net_type).to_string();
                token.show = CTrue;
...
impl ChainTrait for BtcChain {
    fn generate_address(
        ...
        {
            addr.chain_type =
                wallets_types::BtcChain::chain_type(wallet_type, net_type).to_string();
            ...
            let mut master = bitcoin_wallet::account::MasterAccount::from_mnemonic(
                &mnemonic, 0, network, PASSPHRASE, None,
            )
            .map_err(|e| WalletError::Custom(e.to_string()))?;
            let mut unlocker =
                bitcoin_wallet::account::Unlocker::new_for_master(&master, PASSPHRASE)
                    .map_err(|e| WalletError::Custom(e.to_string()))?;
            // path(0,0)
            let account = bitcoin_wallet::account::Account::new(
                &mut unlocker,
...
impl BtcChainTrait for BtcChain {
    async fn get_default_tokens(
        &self,
        context: &dyn ContextTrait,
        net_type: &NetType,
    ) -> Result<Vec<BtcChainTokenDefault>, WalletError> {
        let btc_tokens = BtcChainTokenDefault::list_by_net_type(context, net_type).await?;
        Ok(btc_tokens)
    }

    async fn update_default_tokens(
        &self,
        context: &dyn ContextTrait,
        default_tokens: Vec<BtcChainTokenDefault>,
    ) -> Result<(), WalletError> {
        ...
        for token in default_tokens {
            ...
            {
                let token_shared_wrapper = token_rb.new_wrapper().eq(
                    &MTokenShared::symbol,
                    &token.btc_chain_token_shared.m.token_shared.symbol,
                );
                if let Some(token_shared) =
                    MBtcChainTokenShared::fetch_by_wrapper(token_rb, "", &token_shared_wrapper)
                        .await?
                {
            ...
            {
                let token_default_wrapper = token_rb
                    .new_wrapper()
                ...
                let mut default_token = if let Some(mut token_default) =
                    MBtcChainTokenDefault::fetch_by_wrapper(
                        token_rb,
                        &tx.tx_id,
                        &token_default_wrapper,
                    )
                    .await?
                {
    ...
    async fn update_auth_tokens(
        &self,
        context: &dyn ContextTrait,
        author_tokens: Vec<BtcChainTokenAuth>,
    ) -> Result<(), WalletError> {
        let token_rb = context.db().wallets_db();
        let mut tx = token_rb.begin_tx_defer(false).await?;
        //disable all exist default tokens
        ...
        for token in author_tokens {
            ...
            {
                let token_auth_wrapper = token_rb
                    .new_wrapper()
                    .eq(&MBtcChainTokenAuth::chain_token_shared_id, &shared.id)
                    .eq(&MBtcChainTokenAuth::net_type, &token.net_type);
                //check if this default is exist!
                let mut auth_token = if let Some(mut token_auth) =
                    MBtcChainTokenAuth::fetch_by_wrapper(token_rb, &tx.tx_id, &token_auth_wrapper)
                        .await?
                {
    ...
    async fn get_auth_tokens(
        ...
        start_item: u64,
        page_size: u64,
    ) -> Result<Vec<BtcChainTokenAuth>, WalletError> {
        let btc_tokens =
            BtcChainTokenAuth::list_by_net_type(context, net_type, start_item, page_size).await?;
        Ok(btc_tokens)
    }
    ...
    fn start_murmel(
        ...
        let m_address = {
            ...
            block_on(MAddress::list_by_wrapper(wallet_db, wallet_id, &w))
                .map_err(|e| WalletError::Custom(e.to_string()))?
        };
        let btc_address = m_address
            .iter()
            .filter(|&a| a.chain_type.contains("Btc"))
            .collect::<Vec<&MAddress>>();
        murmel::wallet::start(&net_type, btc_address);
        Ok(())
    }

    async fn load_now_blocknumber(
        &self,
        _context: &dyn ContextTrait,
        net_type: &NetType,
    ) -> Result<BtcNowLoadBlock, WalletError> {
        murmel::wallet::btc_load_now_blocknumber(net_type)
            .await
            .map_err(|e| WalletError::RbatisError(e))
    }
...
impl EeeChainTrait for EeeChain {
    async fn update_basic_info(
        ...
        if basic_info.is_default == CTrue {
            ...
            if let Some(mut current_default_version) =
        ...
        let mut save_basic_info = {
            if let Some(mut existed_info) = SubChainBasicInfo::find_by_version(
                rb,
                &basic_info.genesis_hash,
                basic_info.runtime_version,
                basic_info.tx_version,
            )
            .await?
            {
    ...
    async fn get_basic_info(
        ...
        let chain_version = if genesis_hash.is_empty() && runtime_version == 0 && tx_version == 0 {
            ...
            SubChainBasicInfo::find_by_version(rb, genesis_hash, runtime_version, tx_version)
                .await?
        };
    ...
    async fn get_storage_key(
        ...
        .await?
        {
            ...
            chain_helper
                .get_storage_map_key::<eee::AccountId, u128>(
                    &parameters.module,
                    &parameters.storage_item,
                    account_id,
                )
    ...
    async fn eee_transfer(
        ...
        context: &dyn ContextTrait,
        net_type: &NetType,
        transfer_payload: &EeeTransferPayload,
    ) -> Result<String, WalletError> {
        let data_rb = context.db().data_db(net_type);

        let m_wallet =
            wallets_types::Wallet::find_by_address(context, &transfer_payload.from_account).await?;
        if m_wallet.is_none() {
            return Err(WalletError::Custom(format!(
                " address {} wallet is not exist!",
    ...
    async fn save_tx_record(
        &self,
        context: &dyn ContextTrait,
        net_type: &NetType,
        extrinsic_ctx: &ExtrinsicContext,
    ) -> Result<(), WalletError> {
        let data_rb = context.db().data_db(net_type);
        ...
        let basic_info = self
            .get_basic_info(
                context,
                net_type,
                &extrinsic_ctx.chain_version.genesis_hash,
                runtime_version,
                tx_version,
            )
            .await?;
        ...
        let helper = eee::chain_helper::ChainHelper::init(
            &basic_info.metadata,
            &genesis_byte,
            runtime_version as u32,
            tx_version as u32,
            None,
        )?;
        ...
        let extrinsics_map =
            helper.decode_extrinsics(&extrinsic_ctx.extrinsics, &extrinsic_ctx.account)?;
        ...
        for (index, transfer_detail) in extrinsics_map.iter() {
            ...
            if let Some(is_successful) = event_res.get(index) {
                Self::save_transfer_detail(
                    data_rb,
                    extrinsic_ctx,
                    transfer_detail,
                    tx_time.timestamp.unwrap(),
                    *is_successful,
                )
                .await?;
            }
    ...
    async fn get_tx_record(
        &self,
        context: &dyn ContextTrait,
        net_type: &NetType,
        token_type: EeeTokenType,
        target_account: Option<String>,
        start_item: u64,
        page_size: u64,
    ) -> Result<Vec<EeeChainTx>, WalletError> {
        ...
        let wrapper = {
            if let Some(account) = target_account {
                data_rb
                    .new_wrapper()
                    .eq(MEeeChainTx::wallet_account, &account)
            } else {
                data_rb.new_wrapper()
            }
        };
        ...
        match token_type {
            EeeTokenType::Eee => {
                let eee_tx_record =
                    MEeeChainTx::fetch_page_by_wrapper(data_rb, "", &wrapper, &page_req).await?;
                let res = eee_tx_record
                    .records
                    .iter()
                    .map(|record| EeeChainTx::from(record.clone()))
                    .collect::<Vec<EeeChainTx>>();
                Ok(res)
            }
            EeeTokenType::TokenX => {
                let tokenx_record =
                    MEeeTokenxTx::fetch_page_by_wrapper(data_rb, "", &wrapper, &page_req).await?;
                let res = tokenx_record
                    .records
                    .iter()
                    .map(|record| EeeChainTx::from(record.clone()))
                    .collect::<Vec<EeeChainTx>>();
    ...
    async fn get_default_tokens(
        ...
        context: &dyn ContextTrait,
        net_type: &NetType,
    ) -> Result<Vec<EeeChainTokenDefault>, WalletError> {
    ...
    async fn update_default_tokens(
        &self,
        context: &dyn ContextTrait,
        default_tokens: Vec<EeeChainTokenDefault>,
    ) -> Result<(), WalletError> {
        let token_rb = context.db().wallets_db();
        let mut tx = token_rb.begin_tx_defer(false).await?;
        //disable all exist default tokens
        ...
        for token in default_tokens {
            let mut shared = token.eee_chain_token_shared.clone();
            {
                let token_shared_wrapper = token_rb.new_wrapper().eq(
                    &MTokenShared::symbol,
                    &token.eee_chain_token_shared.m.token_shared.symbol,
                );
                if let Some(token_shared) = MEeeChainTokenShared::fetch_by_wrapper(
                    token_rb,
                    &tx.tx_id,
                    &token_shared_wrapper,
                )
                .await?
                {
                    shared.id = token_shared.id;
            ...
            }
            {
                ...
                //check if this default is exist!
                let mut default_token = if let Some(mut token_default) =
                    MEeeChainTokenDefault::fetch_by_wrapper(
                        token_rb,
                        &tx.tx_id,
                        &token_default_wrapper,
                    )
    ...
    async fn update_auth_tokens(
        &self,
        context: &dyn ContextTrait,
        author_tokens: Vec<EeeChainTokenAuth>,
    ) -> Result<(), WalletError> {
        let token_rb = context.db().wallets_db();
        let mut tx = token_rb.begin_tx_defer(false).await?;
        ...
        for token in author_tokens {
            let mut shared = token.eee_chain_token_shared.clone();
            {
                ...
                if let Some(token_shared) =
                    MEeeChainTokenShared::fetch_by_wrapper(token_rb, "", &token_shared_wrapper)
                        .await?
                {
                    shared.id = token_shared.id;
                }
            ...
            }
            {
                ...
                //check if this default is exist!
                let mut token_auth = if let Some(mut token_auth) =
                    MEeeChainTokenAuth::fetch_by_wrapper(token_rb, &tx.tx_id, &token_auth_wrapper)
                        .await?
                {
                    token_auth.position = token.position;
                    token_auth
    ...
    async fn get_auth_tokens(
        ...
        start_item: u64,
        page_size: u64,
    ) -> Result<Vec<EeeChainTokenAuth>, WalletError> {
        let eee_tokens =
            EeeChainTokenAuth::list_by_net_type(context, net_type, start_item, page_size).await?;
        Ok(eee_tokens)
    }
}

impl EeeChain {
    async fn save_transfer_detail(
        rb: &rbatis::rbatis::Rbatis,
        extrinsic_ctx: &ExtrinsicContext,
        tx_detail: &eee::TransferDetail,
        timestamp: u64,
        is_successful: bool,
    ) -> Result<(), WalletError> {
        log::info!(
            "save_transfer_detail account:{},blockhash:{}",
            extrinsic_ctx.account,
            extrinsic_ctx.block_hash
        );

        match tx_detail.token_name.as_str() {
            "EEE" => {
                Self::save_eee_chain_tx(rb, extrinsic_ctx, tx_detail, timestamp, is_successful)
                    .await?
            }
            "TokenX" => {
                Self::save_eee_tokenx_tx(rb, extrinsic_ctx, tx_detail, timestamp, is_successful)
                    .await?
            }
            _ => {
    ...
    async fn save_eee_chain_tx(
        ...
        let query_tx_wrapper = rb
            .new_wrapper()
            .eq(
                &mav::ma::TxShared::tx_hash,
                tx_detail.hash.clone().unwrap_or_default(),
            )
            .eq(
                &mav::ma::TxShared::signer,
                tx_detail.signer.clone().unwrap_or_default(),
            )
...
impl EthChainTrait for EthChain {
    async fn tx_sign(
        ...
        let pri_key =
            Self::get_private_key_from_address(context, &tx_payload.from_address, password).await?;
        let tx_signed = raw_tx.sign(&pri_key, Some(chain_id));
        Ok(format!("0x{}", hex::encode(tx_signed)))
    }

    async fn raw_tx_sign(
        &self,
        context: &dyn ContextTrait,
        net_type: &NetType,
        raw_tx_payload: &EthRawTxPayload,
        password: &str,
    ) -> Result<String, WalletError> {
        let chain_id = match net_type {
            NetType::Main => 1,
        ...
        let pri_key =
            Self::get_private_key_from_address(context, &raw_tx_payload.from_address, password)
                .await?;
        let tx_signed = raw_tx.sign(&pri_key, Some(chain_id));
        Ok(format!("0x{}", hex::encode(tx_signed)))
    }
    ...
    async fn update_default_tokens(
        &self,
        context: &dyn ContextTrait,
        default_tokens: Vec<EthChainTokenDefault>,
    ) -> Result<(), WalletError> {
        let token_rb = context.db().wallets_db();
        let mut tx = token_rb.begin_tx_defer(false).await?;
        //disable all exist default tokens
        ...
        for token in default_tokens {
            let mut shared = token.eth_chain_token_shared.clone();
            {
                let token_shared_wrapper = token_rb.new_wrapper().eq(
                    &MTokenShared::symbol,
                    &token.eth_chain_token_shared.m.token_shared.symbol,
                );
                if let Some(token_shared) = MEthChainTokenShared::fetch_by_wrapper(
                    token_rb,
                    &tx.tx_id,
                    &token_shared_wrapper,
                )
            ...
            }
            {
                ...
                //check if this default is exist!
                let mut default_token = if let Some(mut token_default) =
                    MEthChainTokenDefault::fetch_by_wrapper(
                        token_rb,
                        &tx.tx_id,
                        &token_default_wrapper,
                    )
    ...
    async fn get_default_tokens(
        ...
        context: &dyn ContextTrait,
        net_type: &NetType,
    ) -> Result<Vec<EthChainTokenDefault>, WalletError> {
    ...
    async fn update_auth_tokens(
        &self,
        context: &dyn ContextTrait,
        author_tokens: Vec<EthChainTokenAuth>,
    ) -> Result<(), WalletError> {
        let token_rb = context.db().wallets_db();
        let mut tx = token_rb.begin_tx_defer(false).await?;
        ...
        for token in author_tokens {
            let mut shared = token.eth_chain_token_shared.clone();
            {
                ...
                if let Some(token_shared) =
                    MEthChainTokenShared::fetch_by_wrapper(token_rb, "", &token_shared_wrapper)
                        .await?
                {
                    shared.id = token_shared.id;
                }
            ...
            }
            {
                ...
                //check if this default is exist!
                let mut token_auth = if let Some(mut token_auth) =
                    MEthChainTokenAuth::fetch_by_wrapper(token_rb, &tx.tx_id, &token_auth_wrapper)
                        .await?
                {
    ...
    async fn update_non_auth_tokens(
        &self,
        context: &dyn ContextTrait,
        author_tokens: Vec<EthChainTokenNonAuth>,
    ) -> Result<(), WalletError> {
        let token_rb = context.db().wallets_db();
        let mut tx = token_rb.begin_tx_defer(false).await?;
        //disable all exist auth tokens
        ...
        for token in author_tokens {
            ...
            {
                let query_wrapper = token_rb
                    .new_wrapper()
                    .eq(&MEthChainTokenNonAuth::chain_token_shared_id, &shared.id)
                    .eq(&MEthChainTokenNonAuth::net_type, &token.net_type);
                //check if this default is exist!
                let mut new_token = if let Some(mut token_non_auth) =
                    MEthChainTokenNonAuth::fetch_by_wrapper(token_rb, &tx.tx_id, &query_wrapper)
                        .await?
                {
    ...
    async fn get_non_auth_tokens(
        &self,
        context: &dyn ContextTrait,
        net_type: &NetType,
    ) -> Result<Vec<EthChainTokenNonAuth>, WalletError> {
    ...
    async fn query_auth_tokens(
        ...
        EthChainTokenAuth::query_by_condition(
            context,
            net_type,
            name,
            contract_addr,
...
impl EthChain {
    async fn get_private_key_from_address(
        context: &dyn ContextTrait,
        address: &str,
        password: &str,
        ...
        if let Some(wallet) = wallets_types::Wallet::find_by_address(context, address).await? {
            let mnemonic =
                eee::Sr25519::get_mnemonic_context(&wallet.mnemonic, password.as_bytes())?;
...
#[allow(dead_code)]
impl BtcChain {
    async fn get_mnemonic_from_address(
        context: &dyn ContextTrait,
        address: &str,
        password: &str,
    ) -> Result<Vec<u8>, WalletError> {
