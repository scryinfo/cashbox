...
pub mod android {
    ...
    #[no_mangle]
    #[allow(non_snake_case)]
    pub extern "C" fn Java_info_scry_wallet_1manager_NativeLib_eeeTxSign(env: JNIEnv, _class: JClass, rawTx:jbyteArray, walletId: JString, psd: jbyteArray) -> jobject {
        let pwd = env.convert_byte_array(psd).unwrap();
        let raw_tx = env.convert_byte_array(rawTx).unwrap();
        ...
        match wallets::module::wallet::raw_tx_sign(raw_tx.as_slice(),&wallet_id,pwd.as_slice()){
            Ok(data)=>{
