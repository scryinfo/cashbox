...
pub fn save_mnemonic(wallet_name:String,mn: &[u8], password: &[u8]) -> Result<Mnemonic,String> {
    ...
    match instance {
        Ok(mut dataservice)=>{
            ...
            dataservice.save_mnemonic_address(mnemonic_save,address_save);
            Ok(Mnemonic {
                status: StatusCode::OK,
...
struct KeyCrypto {
    ...
    kdf: String,//使用的kdf算法 现在默认使用scrypt
    kdfparams: KdfParams,
    mac: String,
}
