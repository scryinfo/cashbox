...
use mav::ma::{Db, EeeTokenType, MAddress, MWallet};

use crate::{AccountInfo, AccountInfoSyncProg, BtcBalance, BtcChainTokenAuth, BtcChainTokenDefault, BtcNowLoadBlock, BtcTxParam, DecodeAccountInfoParameters, EeeChainTokenAuth, EeeChainTokenDefault, EeeChainTx, EeeTransferPayload, EthChainTokenAuth, EthChainTokenDefault, EthChainTokenNonAuth, EthRawTxPayload, EthTransferPayload, EthWalletConnectTx, ExtrinsicContext, RawTxParam, StorageKeyParameters, SubChainBasicInfo, WalletError};
...
pub trait BtcChainTrait: Send + Sync {
    ...
    async fn load_now_blocknumber(&self, context: &dyn ContextTrait, net_type: &NetType) -> Result<BtcNowLoadBlock, WalletError>;
    async fn load_balance(&self, context: &dyn ContextTrait, net_type: &NetType) -> Result<BtcBalance, WalletError>;
