...
pub unsafe extern "C" fn Java_info_scry_wallet_1manager_NativeLib_mnemonicGenerate(env: JNIEnv, _: JClass, count: jint) -> jobject {
    ...
    if count>127{
        ...
        env.set_field(mn_obj, "message", "Ljava/lang/String;", JValue::Object(JObject::from(env.new_string("please input correct mnemonic num").unwrap()))).expect("isContainWallet set message ");
    }else{
...
fn wallet_jni_obj_util<'a, 'b>(env: &'a JNIEnv<'b>, wallet: Wallet) -> JObject<'a> {
    ...
    let display_chain_id_obj = JObject::from(display_chain_id_str);
    ...
    env.set_field(jobj, "nowChainId", "Ljava/lang/String;", JValue::Object(display_chain_id_obj)).expect("nowChainId");
    ...
    match wallettype {
        ...
        Err(e) => {
            log::info!("set wallet type error {}", e.to_string());
        }
    }
